...
At this point, the :ref:`railway-traffic-control` has already computed a schedule for all trains in the network. This schedule is transmitted to train onboard computers through the railway communications infrastructure and all trains must attain to it as much as possible. This section introduces all the agents involved in train automation and some basic :term:`ATC` mathematical models.
...
The chapter is structured as follows. First, the mathemateical model for the train motion is introduced in :ref:`train-dynamic-models`. Next, the speed profile optimization problem is introduced and stated rigurously in :ref:`speed-profile-optimization`. Finally, a simple optimal control model is introduced in :ref:`optimal-train-control` to drive the train according to the optimal speed profile.
