...
class Train:
    """Train class implementing the model described in documentation (see #TODO: Write reference here).
    ...
    The motion of the train can be described by Newton's law:
        m * rho * (dv/dt) = u(t) - Rb(v) - Rline(x, v), where
            ...
            - Rb(v): Basic resistance of the train at a given speed.
                Rb(v) = m * (a1 + a2 * v**2), where a1 and a2 coefficients depend on the train characteristics and the
                wind speed.
            - Rline(x, v): Line resistance of a train at a given position and speed. It is the addition of the slope
            resistance at a given point, the curve resistance at a given point and the tunnel resistance at a given
            point and velocity.
    ...
    def __init__(self, mass, massfactor, velocity, max_brake, max_traction, basic_resistance_params):
        """Initialise train object:
            ...
            Args:
                ...
                emergency brake.
                max_traction (float): Maximum traction force in [Newtons].
